...
function regexFallback(query) {
  ...
  return { budget: parseBudget(query), walkMax: parseWalkMax(query), tags, reason: "정규식 폴백 해석", source: "regex-fallback" };
}
  ...
  if (apiKey) {
    try {
      ...
      if (up.ok) {
        ...
        const budget = typeof parsed.budget === "number" && parsed.budget > 0 ? Math.round(parsed.budget) : null;
        const walkMax = typeof parsed.walkMax === "number" && parsed.walkMax > 0 ? Math.round(parsed.walkMax) : null;
