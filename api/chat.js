...
6) 데이터에 없는 걸 물으면 모른다고 말하고 search=false 로 둔다.
   **특히 <데이터> 식당 목록에 없는 가게 이름을 물으면("맥도날드 메뉴 뭐 있어?")
   "그 가게는 아직 없습니다" 라고 그 가게를 짚어 답한다.** 딴 가게나 학식 이야기로 돌리지 마라 —
   묻지 않은 걸 답하는 것도 틀린 답이다.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const reply = String(p.reply ?? "").replace(/<\/?데이터>/g, "").replace(/\s{2,}/g, " ").trim();
