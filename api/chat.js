...
import { pickChips, mergeConditions } from "./_lib/chips.js";
import { extractFoods, verifyFoods, stripFabricatedMemory } from "./_lib/memory.js";
...
export function extractConditions(message) {
  ...
  const tags = ["혼밥", "가성비", "데이트", "회식", "단체", "격식", "해장", "야식", "양많음", "시험기간", "초밥", "김밥", "쌀국수", "치킨", "생선", "돈카츠"]
    .filter((t) => {
      let i = s.indexOf(t);
      while (i !== -1) {
...
3) reply 는 한국어 **최대 2문장, 80자 이내**. **추천 식당 이름은 절대 대지 마라** —
   어디를 고를지는 네가 아니라 화면 아래 카드가 정한다. 네가 이름을 대면 카드와 어긋난다.
   조건을 확인하고 "조건에 맞는 곳을 아래 카드에 골라 놓을게요"처럼 카드로 안내한다.
   (사용자가 지목한 목록에 없는 가게를 규칙 6 으로 되짚는 경우는 예외 — 그건 추천이 아니다.)
...
4-1) tags 는 아래 목록에 있는 말만 쓴다. 해당 없으면 빈 배열로 둔다. 음식 분류(한식·중식 등)는 tags 가 아니다.
   허용 tags: 혼밥, 가성비, 데이트, 회식, 단체, 격식, 해장, 야식, 양많음, 시험기간, 초밥, 김밥, 쌀국수, 치킨, 생선, 돈카츠 (격식=교수님·과선배·어른과 식사)
5) 조건이 없는 인사·잡담이면 conditions 는 비우고 search=false 로 두되, 무엇을 물으면 되는지 한 문장으로 안내한다.
   단 <지난방문> 이 있으면 안내 대신 **규칙 9 를 먼저 따른다** — 물어볼 것을 되묻기보다 지난 관심을 꺼내는 편이 낫다.
...
8) chips — 사용자가 **다음에 눌러서 이어갈 만한 질문** 후보를 5개까지 낸다. 서버가 데이터로
   검증해 살아남은 3개만 화면에 나가므로, 아깝다고 아무거나 채우지 말고 진짜 후보만 낸다.
   - label 은 사용자가 하는 말투로 14자 이내 (예: "도보 5분 안으로", "더 싼 곳으로").
   - 조건은 label 과 일치해야 한다. budget/walkMax 는 숫자, tags 는 4-1 허용 목록만.
     해당 없으면 null / 빈 배열.
   - **지금 걸린 조건을 그대로 반복하지 마라** — 이미 답한 질문을 다시 권하는 셈이다.
   - <데이터>에 없는 성질(24시·배달·주차·평점)은 제안하지 마라. 눌러도 그런 결과가 없다.
   - 오늘 학식이 열려 있으면 학식으로 넘어가는 칩 하나는 조건 없이 내도 된다.
9) <지난방문> 이 주어졌고 사용자가 **무엇을 먹을지 아직 안 정한 채** 물으면("오늘 뭐 먹지",
   "뭐 먹을까", "배고파"), **그 관심을 첫 문장에서 반드시 꺼낸다** — "지난번엔 OO 찾으셨는데,
   오늘도 그쪽으로 볼까요?" 사용자가 이미 다른 것을 지목했으면 꺼내지 마라.
   <지난방문> 에 없는 것을 지난번에 물었다고 말하는 것은 없는 기억을 지어내는 것이다.`;

export default async function handler(req, res) {
  res.setHeader("access-control-allow-origin", "*");
  if (req.method !== "POST") return res.status(405).json({ error: "POST only", code: "method_not_allowed" });

  // conditions: 칩을 눌러 들어온 턴이다. 이미 검증된 조건이므로 문장에서 다시 뽑지 않는다 —
  // 다시 뽑으면 "도보 5분 안으로" 라벨에서 예산이 사라져 조건이 조용히 풀린다.
  // memory: 브라우저가 들고 있던 지난 방문 관심어. 서버는 저장하지 않는다 — 받아서 다시
  // 검증하고 이번 턴에만 쓴다(ADR-0004 §4 무상태).
  const { message = "", history = [], conditions = null, memory = [] } = req.body ?? {};
  ...
  let context = null, data = null, cafeteriaOpen = false;
  try {
    ...
    cafeteriaOpen = data.cafeteria.some((c) => c.menu_date === todayISO && c.items?.length);
  } catch {
  ...
  // 기억은 **꺼낼 때마다** 현재 데이터로 다시 거른다. 사라진 메뉴를 계속 말하면 그때부터 거짓말이다.
  const pastFoods = verifyFoods(memory, data);
  const nowFoods = extractFoods(message, data);
  const given = conditions && typeof conditions === "object" ? mergeConditions(conditions, {}) : null;
  const code0 = given ?? extractConditions(message);
  ...
  const msgs = [
    ...
    // 지난 방문 관심 — 데이터 대조를 통과한 낱말만 들어온다(ADR-0004 §3). 문장은 모델이 쓴다.
    ...(pastFoods.length ? [{
      role: "system",
      content: `<지난방문>이 사용자가 지난 방문에 물었던 것: ${pastFoods.join(", ")}</지난방문>\n`
        + `사용자가 **무엇을 먹을지 아직 안 정한 상태**로 물으면(예: "오늘 뭐 먹지", "뭐 먹을까")\n`
        + `이 관심을 먼저 꺼내 제안한다 — "지난번엔 ${pastFoods[0]} 찾으셨는데, 오늘도 그쪽으로 볼까요?" 처럼.\n`
        + `이미 다른 것을 지목했으면 억지로 끼워넣지 마라.\n`
        + `이 목록에 없는 것을 지난번에 물었다고 말하지 마라 — 없는 기억을 지어내는 것이다.`,
    }] : []),
  ];
  ...
  try {
          ...
          json_schema: {
            ...
            schema: {
              ...
              properties: {
                ...
                // 다음 턴 후보(칩). 서버가 실데이터로 검증해 3개로 줄인다 — 여기 값은 후보일 뿐이다.
                chips: {
                  type: "array",
                  items: {
                    type: "object",
                    ...
                    required: ["label", "budget", "walkMax", "tags"],
                  },
                },
              },
              required: ["reply", "search", "budget", "walkMax", "tags", "unknownPlace", "chips"],
            },
          },
        },
      }),
    ...
    // 없는 기억을 말하는 문장은 여기서 걸러진다 (ADR-0004 §4③ — 규칙은 `_lib/memory.js`).
    reply = stripFabricatedMemory(reply, pastFoods) || "오늘은 뭘 드시고 싶으세요?";
    ...
    // 칩은 모델이 쓰고 데이터가 판정한다 — 검증에서 살아남은 것만 나간다(`_lib/chips.js`).
    const chips = pickChips(data, p.chips, { budget, walkMax, tags }, { cafeteriaOpen });
