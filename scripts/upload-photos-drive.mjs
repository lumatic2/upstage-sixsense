 ...
 *  이미 같은 이름이 폴더에 있으면 스킵 (idempotent).
 *  사용법: node scripts/upload-photos-drive.mjs  (photo-manifest.json 기준)
 */
...
const manifest = JSON.parse(fs.readFileSync("scripts/photo-manifest.json", "utf8"));
...
for (const ph of manifest.photos) {
  ...
  gws(`gws drive files create --params '{"uploadType":"multipart"}' --json '${body}' --upload '${posix}'`);
  console.log(`업로드: ${drvName}`);
  up++;
}
