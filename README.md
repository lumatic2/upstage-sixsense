...
2. **근거 판정 게이트** — 추천 이유를 쓴 모델이 스스로 채점하면 검증이 아닙니다. **생성(`solar-pro2`)과 판정(`solar-mini`)을 다른 모델이 맡아**
   이유 문장을 실데이터와 대조하고, 근거가 없으면 그 문장을 버리고 데이터 사실로 교체합니다.
   화면에서는 대화창이 몇 건이 통과했는지 알리고, 교체된 카드에만 "근거 미달 판정 → 데이터 팩트로 교체" 배지가 붙습니다.

파이프라인을 직접 돌려보는 페이지가 서비스 안에 있습니다 → **[/about.html](https://sixsense.askewly.com/about.html)** (읽는 자리에서 진짜 API가 돕니다)
...
| 사진 상태 | 정확도 |
|---|---|
| 인쇄체 · 해상도 충분 | 100% (37/37) |
| 표기 변형 · 손글씨 | 64.0% (16/25) |
| 저해상 흐림 | 20.0% (2/10) |

**지배 변수는 해상도 하나였습니다.** 유리창에 반사까지 얹혀 멀리서 찍은 사진도 전량 맞았고, 무너지는 건
해상도가 부족할 때뿐이었습니다. 그래서 "잘 찍어 달라"가 아니라 **"해상도만 확보하면 된다"**가 수집 지침이
됐고, 이 실패 모드가 "전량 자동 반영은 불가능 → 검수 UX 필수"라는 설계 경계를 만들었습니다.
원본 데이터는 `experiments/parse-poc/accuracy.md`.

## 현재 데이터

식당 50곳 · 메뉴 580행(전량 검수 통과) · 학식 4곳. 정본은 팀 구글 시트입니다.
(파싱 후보 719개 중 검수를 통과한 580개만 나갑니다 — 갱신되는 실측치는 `docs/SERVICE-SPEC.md` §8.)

**키 없이 지금 바로 읽을 수 있습니다:**

```bash
curl https://sixsense.askewly.com/api/data
# → { restaurants: 50, menus: 580, cafeteriaBoard: [...], today: "...", stats: {...} }
```
...
node scripts/demo-smoke.mjs --url https://sixsense.askewly.com   # 배포본 E2E (16항목)
...
| `public/` | 화면 5종 (바닐라 JS, 빌드 없음) — `index`(랜딩) `app`(서비스) `about`(파이프라인 설명·실행 증거) `contribute`(제보) `review`(검수). 구 `verify`는 `about`으로 합쳐 리다이렉트만 남음 |
...
- **[docs/SERVICE-SPEC.md](docs/SERVICE-SPEC.md)** — 서비스 전체 정리(문제·파이프라인·데이터·Upstage·실측 수치). 한 문서로 무엇을·어떻게·어떤 숫자를 대는지 본다
