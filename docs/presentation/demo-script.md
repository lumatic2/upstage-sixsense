...
> 시연 대상은 라이브 서비스 https://sixsense.askewly.com 이다. (구 "hanipmap 통합본으로 교체 예정" 은 2026-07-20 폐기 — `docs/adr/0003-no-assembly-build.md`)

## 시나리오 (약 4분)

| # | 단계 | URL·입력 | 기대 출력 | 실패 시 폴백 |
|---|---|---|---|---|
| 1 | 데모 지도 열기 | sixsense.askewly.com | 명륜캠 지도 + 식당 24곳 | 사전 캡처 화면 |
| 2 | 자연어 검색 | 스테이징 `/verify.html` §② — "과선배가 사는 날인데 2만원으로 둘이" | `{budget:10000, tags:["격식"], source:"solar"}` — 인원 나눗셈 강조 | **폴백이 기능이다**: Solar 죽어도 정규식 경로로 검색 동작 — `source:"regex-fallback"` 을 그대로 보여주며 설계 설명 |
