...
사용자가 업종(일식·한식·양식·중식·분식·아시안)을 지정하면 그 분류만 남기는 **사전 필터**를 먼저 건다 — 태그(혼밥·초밥)는 가점이지만 분류는 필터다(시트 분류값이 "일식/스시"처럼 합성이라 substring 매칭). 추출은 `extractConditions` 가 코드로 하고(모델 아님, 어절-앞머리 규칙) 프런트 `state.category`→body 로 흐른다.
남은 후보를 `태그 일치 > 예산 활용도 > 예산 내 본메뉴 수 > 도보 시간` 으로 정렬한다.
예산 활용도(예산 내 최고가÷예산)는 예산을 높게 부를 때 저가 분식이 메뉴 수만으로 상단을 차지하지 않게, 예산을 알차게 쓰는 집을 위로 올린다. 예산은 사용자가 준 명시 조건이라 개인화 배제 원칙(ADR-0004)과 무관하다.
